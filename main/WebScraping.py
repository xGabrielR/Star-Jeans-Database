...
class WebScraping():
    ...
    def __init__( self ):
        ...
        self.database = 'sqlite:///h&m_database.sqlite'
    ...
    def data_cleaning( self, df, logger ):
        ...
        df = df[['Fit', 'Composition', 'Description', 'Art. No.', 'Price']]
    ...
    def data_preparation( self, df_clean, df_ref, logger ):
        try:
            ...
            df['name'] = df['fit'].apply( lambda x: x.replace('fit', 'jeans'))
            df = df[['product_id', 'name', 'color', 'fit', 'price', 'cotton', 'polyester', 'spandex', 'elasterell', 'date']]
    ...
    def data_storange( self, df, logger ):
        try:
            ...
            #df.to_csv('h&m_datasets/h&m_' + df['date'][0] + '.csv' )
            
            df1 = pd.read_sql_query( 'SELECT * FROM showroom', con=con )

        except ValueError:
            logger.error('[ERROR] Cannot Store Dataset at Database on data_storange.')
            
        return df1.to_json( orient='records', date_format='iso' )